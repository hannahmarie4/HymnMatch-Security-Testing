...
export default function Home() {
  ...
  // Flow state machine
  // 'idle' | 'blur-checking' | 'blur-error' | 'preview' | 'file-error' | 'confirming'
  ...
  const onConfirm = () => {
    ...
    reader.onloadend = () => {
      sessionStorage.setItem('hymnmatch_upload', JSON.stringify({
  ...
  const FileErrorOverlay = () => (
    <ModalWrapper>
      <div className="flex flex-col items-center text-center space-y-4 py-4">
        ...
        <h3 className="text-xl font-extrabold text-slate-800 dark:text-slate-100">Invalid File</h3>
  ...
  return (
    <div className="max-w-4xl mx-auto py-4">
      ...
      <input
        ...
        className="hidden"
      />
      {/* Any device: file picker (PDF + images) */}
      <input
        type="file"
      ...
      <div className="mb-10">
        ...
        {/* Drop zone hint */}
        <div className="bg-white/60 dark:bg-slate-900/40 backdrop-blur-sm border-2 border-dashed border-purple-200 dark:border-purple-900/50 rounded-3xl p-10 flex flex-col items-center justify-center mb-6 transition-all">
          ...
          <p className="text-slate-800 dark:text-slate-200 font-bold text-xl mb-1">
        ...
        {/* Two action buttons — side-by-side on desktop, stacked on mobile */}
        <div className={`grid gap-4 ${isMobile ? 'grid-cols-1' : 'grid-cols-2'}`}>
          {/* Upload Image */}
          <button
            onClick={onClickUploadImage}
            className="flex items-center justify-center space-x-3 py-4 px-6 rounded-2xl bg-gradient-to-r from-purple-600 to-indigo-600 text-white font-bold text-base hover:shadow-lg hover:shadow-purple-500/30 dark:hover:shadow-purple-900/50 transform hover:-translate-y-0.5 transition-all"
          >
            <FiCamera size={22} />
